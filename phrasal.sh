...
#
function make_ini_from_batch_run {
...
#
function make_ini_from_online_run {
    # Setup the final weights and ini files
...
do
  ...
  if [ $step -eq 4 ]; then
	  step_status "$step -- Decode test set"
	  
    if [ $TUNE_MODE == "batch" ]; then
	    make-ini-from-batch-run
	  else
	    make_ini_from_online_run
	  fi
	  decode
  fi
